...
implements JGEngineInterface  {
	...
	class JGView extends View implements SensorEventListener {
		...
		* Sensitivity should be a setting. Unclear how to test button press. */
		...
		private void handleKey(int key,boolean state) {
			switch (key) {
			case KeyEvent.KEYCODE_0 : keymap['0'] = state; break;
			}
	...
	void setColor(Paint p,JGColor col) {
		p.setColor(0xff000000 | (col.r<<16) | (col.g<<8)  | col.b);
	}
	...
	* passing a null means ignore that argument. */
	...
	boolean pf_relative) {
		...
		if (img!=null)
			g.drawBitmap(img.img,(int)x,(int)y,bufpainter);
	}
	...
	double alpha, double rot, double scale, boolean pf_relative) {
		...
		JGPoint size = el.scalePos(origsize.x,origsize.y,false);
		// size of scaled image
		double txsize_s = size.x*scale;
		double tysize_s = size.y*scale;
		// determine topleft of scaled image
		//System.err.println(""+size.x+" "+size.y+" "+txsize_s+" "+tysize_s);
		//x -= (txsize_s - size.x)/2;
		//y -= (tysize_s - size.y)/2;
		float pivotx = (float)(origsize.x/2.0);
		float pivoty = (float)(origsize.y/2.0);
		Matrix m = new Matrix();
		m.setRotate((float)((-rot*180)/Math.PI),pivotx,pivoty);
		m.postScale((float)scale,(float)scale, pivotx,pivoty);
		m.postScale((float)el.x_scale_fac,(float)el.y_scale_fac, 0,0);
		m.postTranslate((float)x,
			(float)y);
		if (img!=null)
			buf_gfx.drawBitmap(img.img,m,imagepainter);
	}
...
//	} }
	...
	public boolean storeExists(String id) {
		return store_data.containsKey(id);
	}
