...
public class Ogrotron extends StdGame {
	...
	public void paintFrameInGame() {
		...
		&& show_man_accel==0
		&& (Math.abs(getAccelZ()) 
			< Math.max(Math.abs(getAccelX()),Math.abs(getAccelY()))) 
		) {
			show_man_accel=4*50;
		}
	...
	void explode(double x,double y) {
		...
		if (isOpenGL() || isAndroid()) {
			...
			new JGObject("explo",true, x,y, 0,"explo_u",0,-4,
				JGObject.expire_off_pf);
			new JGObject("explo",true, x,y, 0,"explo_d",0,4,
				JGObject.expire_off_pf);
		}
	}
	...
	public class Player extends JGObject {
		...
		public void move() {
			...
			if (hasAccelerometer() && (getAccelX()!=0 || getAccelY()!=0)) {
				double angle = atan2(getAccelX(),getAccelY());
				xspeed = 2.0*getAccelX();
				yspeed = 2.0*getAccelY();
				xdir = 1;
				ydir = 1;
			}
			if (x < 8) {
